...
# Increase dimensions of the cave by 5x5
new_grid = [[0] * (5 * len(grid)) for _ in range(5 * len(grid))]
for i in range(5):
    for j in range(5):
        risk_increase = i + j
        gl = len(grid)
        for x in range(gl):
            for y in range(gl):
                new_val = grid[x][y].val + risk_increase
                new_val -= 9 if new_val > 9 else 0
                new_grid[x + i * gl][y + j * gl] = Chiton(new_val)

cavern = Cavern(new_grid)
